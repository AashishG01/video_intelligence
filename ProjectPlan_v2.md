...
│  │  BODY Re-ID: OSNet / BoT-TransReID / FastReID       │              │
...
│  ├── Primary collection: all face/body embeddings                     │
...
| `sightings` | id, person_id, camera_id, timestamp, bbox, confidence, thumbnail_path, video_clip_path, embedding_type [face\|body\|combined] | Hypertable (TimescaleDB), partitioned by time |
...
| Person Re-ID | **OSNet + BoT-TransReID + FastReID** | Ensemble body matching — clothing, gait, body shape |
...
| Challenge | Solution |
|---|---|
| **Auto-rickshaw occlusion** (blocks 30-50% of view) | Body Re-ID fallback. Track across frame continuity. Accept lower yield from rickshaw-heavy cameras. |
| **Dupatta / Hijab / Chunni** (half-face) | AdaFace handles partial faces. Body Re-ID as fallback. |
| **Helmeted two-wheeler riders** (~30% of people) | Cannot face-match. Document as known limitation. Body/vehicle Re-ID where possible. |
...
| **School uniforms / factory workers** | Face-only matching for uniform groups. Body Re-ID unreliable when clothing identical. |
...
| No Re-ID | OSNet/BoT-TransReID/FastReID ensemble | 🟡 High |
...
│   ├── embedding/              # RetinaFace + AdaFace + Re-ID
...
- [ ] Download & test OSNet for person Re-ID (512-d body embedding, <5ms)
...
- [ ] Multi-GPU assignment: GPU 0-1 (YOLO detection), GPU 2 (face/body embedding), GPU 3 (Milvus + watchlist)
...
| Accuracy below 75% on real footage | High | Medium | Already proven 86% in POC. Upgrade to AdaFace + Re-ID ensemble for higher. |
| **Indian crowd density overwhelms GPU** | **High** | **High** | **Tiered processing + temporal dedup + ROI filtering. Start 4×A100, scale to 8×.** |
| Auto-rickshaw occlusion drops face yield | High | High | Body Re-ID fallback. Accept 5-25% face yield on vehicle-heavy cameras. |
