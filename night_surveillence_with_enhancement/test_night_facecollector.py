...
# ── Camera type flag ──────────────────────
# Set to True  → high-angle street camera (loose thresholds, upscaling)
# Set to False → face-level camera (stricter thresholds, no upscaling)
HIGH_ANGLE_CAMERA = True

# ── Thresholds auto-set from camera type ─
if HIGH_ANGLE_CAMERA:
    DET_SCORE_THRESHOLD = 0.35   # looser — catch partial/angled faces
    MIN_FACE_SIZE       = 50     # smaller — distant faces are tiny
    UPSCALE_FACTOR      = 1.5    # enlarge frame before detection
    DET_SIZE            = (1024, 1024)
else:
    DET_SCORE_THRESHOLD = 0.55   # stricter — only good frontal faces
    MIN_FACE_SIZE       = 60     # larger — face-level cams are closer
    UPSCALE_FACTOR      = 1.0    # no upscaling needed
    DET_SIZE            = (1024, 1024)
...
def denoise(frame):
    # Bilateral filter — removes grain while keeping edges sharp
...
# Lower internal detection threshold so the model attempts
# face crops it would normally skip (key for small/angled faces)
